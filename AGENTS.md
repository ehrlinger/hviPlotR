...
`push` to `main`/`master` and `workflow_dispatch`; there is no
`pull_request`. So a raw-Unicode `.Rd` passes every check a PR runs and
only turns `main` red *after* the merge. If a change touches `man/` or
any roxygen block, build the manual yourself before handing the PR over:
...
- **Bump when you name a version, not when you merge.** A pull request
  ...
  `# hvtiPlotR (unreleased)` heading in `NEWS.md`, which you add when it
  is not already there. A separate commit then renames that heading to
  the new version and updates `DESCRIPTION` and its `Date`, at most once
  a day. The heading is gone again after a bump, so the next change
